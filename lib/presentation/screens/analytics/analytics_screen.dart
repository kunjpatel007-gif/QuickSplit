...
class _AnalyticsScreenState extends State<AnalyticsScreen> with SingleTickerProviderStateMixin {
          ...
          : SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  Row(
                    children: [
                      Expanded(child: _buildSummaryCard('Total', totalSpending, context)),
                      const SizedBox(width: 8),
                      Expanded(child: _buildSummaryCard('Average', avgExpense, context)),
                    ],
                  ),
                  const SizedBox(height: 8),
                  Row(
                    children: [
                      Expanded(child: _buildSummaryCard('Count', expenses.length.toDouble(), context, isCount: true)),
                      const SizedBox(width: 8),
                      Expanded(child: _buildSummaryCard('Top Cat', 0, context, textValue: mostUsedCategory)),
                    ],
                  ),
  ...
  Widget _buildSummaryCard(String title, double amount, BuildContext context, {bool isCount = false, String? textValue}) {
    final tt = Theme.of(context).textTheme;
    return Card(
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            Text(
              textValue ?? (isCount ? amount.toInt().toString() : CurrencyFormatter.format(amount)),
  ...
  Widget _buildLegend(Map<String, double> categoryTotals, double totalAmount, TextTheme tt) {
    return Column(
        ...
        return Padding(
          ...
          child: Row(
            children: [
              ...
              Text('${CurrencyFormatter.format(e.value)} (${percentage.toStringAsFixed(1)}%)', style: tt.bodyMedium),
            ],
          ),
        );
      }).toList(),
  ...
  Widget _buildTopSpendersList(Map<int, double> balances, dynamic userProvider, TextTheme tt) {
...
class DonutChartPainter extends CustomPainter {
  ...
  DonutChartPainter({required this.categoryTotals, required this.totalAmount, required this.progress, required this.onSurface});
...
class BarChartPainter extends CustomPainter {
  ...
  BarChartPainter({required this.expenses, required this.progress, required this.primary, required this.onSurface});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final paint = Paint()..style = PaintingStyle.fill..color = primary;
    ...
    for (int i = 0; i < 7; i++) {
      ...
      final textPainter = TextPainter(
        text: TextSpan(text: dayLabels[i], style: TextStyle(color: onSurface, fontSize: 10)),
        textDirection: TextDirection.ltr,
      );
      textPainter.layout();
